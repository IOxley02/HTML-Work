...
var pointsArray = [];
var normalsArray = [];
var shapePosition = vec3(0, 0, 0);
var shapeVelocity = vec3(0.003, 0.003, 0.003);
var shape = 'torus';
var isFirst = true;
...
function render() {
    ...
    gl.useProgram(program);
    shapePosition = add(shapePosition, shapeVelocity);
    if(shapePosition[0] > 0.75 || shapePosition[0] < -0.75) {
        randomize(1, 2);
        if(shapePosition[0] > 0.75) {
            shapeVelocity[0] = -1 * (Math.random() * (0.005 - 0.0015) + 0.0015);
        } else {
            shapeVelocity[0] = (Math.random() * (0.005 - 0.0015) + 0.0015);
        }
    }
    if(shapePosition[1] > 0.75 || shapePosition[1] < -0.75) {
        randomize(0, 2);
        if(shapePosition[1] > 0.75) {
            shapeVelocity[1] = -1 * (Math.random() * (0.005 - 0.0015) + 0.0015);
        } else {
            shapeVelocity[1] = (Math.random() * (0.005 - 0.0015) + 0.0015);
        }
    }
    if(shapePosition[2] > 0.75 || shapePosition[2] < -0.75) {
        randomize(0, 1);
        if(shapePosition[2] > 0.75) {
            shapeVelocity[2] = -1 * (Math.random() * (0.005 - 0.0015) + 0.0015);
        } else {
            shapeVelocity[2] = (Math.random() * (0.005 - 0.0015) + 0.0015);
        }
    }
    
    //draw the minmax box
    modelViewMatrix = lookAt(vec3(viewer.eye), viewer.at, viewer.up);
    gl.uniform1i( gl.getUniformLocation(program, "makeBox"),1 );
    gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(modelViewMatrix) );
    gl.drawArrays( gl.LINES, 0, numBoxVerticies);
    
    gl.uniform1i( gl.getUniformLocation(program, "colorFlag"),1 );
    gl.uniform1i( gl.getUniformLocation(program, "makeBox"),0 );
    modelViewMatrix = mult(translate(shapePosition[0], shapePosition[1], shapePosition[2]), modelViewMatrix);
    
    modelViewMatrix = mult(scalem(Math.abs(shapePosition[0]) + 0.25, 
    Math.abs(shapePosition[1]) + 0.25, 
    Math.abs(shapePosition[2]) + 0.25), modelViewMatrix);
    /*This is my attempt to get it to scale with velocity, but it wasn't behaving well
    modelViewMatrix = mult(scalem(Math.abs(shapePosition[0] + shapeVelocity[0] * 100) + 0.25, 
    Math.abs(shapePosition[1] + (shapeVelocity[1] * 100)) + 0.25, 
    Math.abs(shapePosition[2] + (shapeVelocity[2] * 100)) + 0.25), modelViewMatrix);
    */

    gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(modelViewMatrix) );
    for( var i = numBoxVerticies; i < numVertices + numBoxVerticies; i+=3) {
    ...
    gl.uniform1i( gl.getUniformLocation(program, "makeBox"),0 );
	gl.drawArrays( gl.POINTS, numBoxVerticies + numVertices, 1);
